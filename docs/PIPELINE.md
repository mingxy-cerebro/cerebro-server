# Cerebro — Memory Pipeline Architecture

> Technical deep-dive into how Cerebro stores, retrieves, and evolves memories.
...
Cerebro provides three ingestion paths, each optimized for different use cases:
...
Cerebro integrates with AI coding platforms through four plugins, each adapted to the platform's extension model:
...
**Installation:** `/plugin marketplace add mingxy-cerebro/cerebro-server` or `/plugin install mingxy-cerebro@cerebro`
...
| Defaults | 3rd | `http://localhost:8080`, `""` |
...
**MemoryBackend:** `OmemMemoryBackend` class proxies `store()`, `search()`, `get()`, `update()`, `delete()`, `list()` directly to the Cerebro API.
