# Cerebro Hosted Setup Guide

API Base: `http://localhost:8080`

Cerebro is a self-deployed memory server. No cloud dependency — everything runs on your infrastructure. You need Docker (or the binary) to get started.
...
> Cerebro plugin installed. Next I need to restart Claude Code so the memory plugin loads and the hooks become active.
...
> **Alternatives:** `~/.config/cerebro/config.json` (global config), or `export OMEM_API_KEY=...` env vars as fallback.
...
`[AGENT]` Add the Cerebro MCP server to the client's config file.
...
Double-check the API URL is `http://localhost:8080` (or your server address).
...
curl -sX POST http://localhost:8080/v1/memories \
  ...
  -d '{"content": "Cerebro setup complete", "tags": ["test"]}'

# Search it back
curl -s "http://localhost:8080/v1/memories/search?q=setup+complete&limit=1" \
