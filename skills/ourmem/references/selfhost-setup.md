# Cerebro Self-Hosted Setup Guide
...
docker run -d --name omem -p 8080:8080 ghcr.io/mingxy-cerebro/cerebro-server:latest
...
Download from the [GitHub releases page](https://github.com/mingxy-cerebro/cerebro-server/releases):

```bash
# musl static binary (runs on any Linux x86_64, zero dependencies)
curl -LO https://github.com/mingxy-cerebro/cerebro-server/releases/latest/download/omem-server-vX.Y.Z-linux-amd64.tar.gz
...
By default Cerebro stores data on local disk. For durability and scalability, configure object storage:
...
curl -sX POST http://localhost:8080/v1/memories \
  ...
  -d '{"content": "Cerebro self-hosted setup complete", "tags": ["test"]}'
