...
version: 1.1.0
description: |
  Shared persistent memory for AI agents — self-deployed, full control.
  Collective intelligence with Space-based sharing across agents and teams.

  Use when users say:
  - "install cerebro" / "install memory server"
  - "setup memory" / "setup persistent memory"
  - "add memory plugin"
  - "cerebro onboarding" / "memory onboarding"
  - "memory not working"
  - "remember this"
  - "save this for later"
  - "don't forget"
  ...
  Even if the user doesn't say "cerebro", trigger when they want persistent memory,
...
# Cerebro — Shared Persistent Memory for AI Agents

Cerebro gives AI agents shared persistent memory across sessions, devices, agents, and teams. One API key reconnects everything. Self-deployed for full control over your data.
...
**Do NOT use for:** temporary conversation context, one-off tasks, or troubleshooting unrelated to Cerebro.
...
If the user explicitly asks to remember something and Cerebro is not installed, suggest: "I can set up Cerebro so I'll remember this across sessions. Takes about 2 minutes. Want to do it now?"
...
| Term | Meaning |
|------|---------|
| `apiKey` / `OMEM_API_KEY` / `API key` / `secret` | All refer to the same Cerebro identifier. Prefer "API key" with users. |
| `tenant` | The workspace behind an API key. Don't use this term with users. |

Security: Treat the API key like a secret. Anyone who has it can access that Cerebro space.
...
> Cerebro is a self-deployed memory server. You'll need Docker (or the binary) to run it.
> >
> Already have a Cerebro server running? Paste your API key and I'll reconnect you.

### Setup instructions

- **Self-hosted** → READ `references/selfhost-setup.md` for server deployment + setup
- **Existing key** → Verify: `curl -sf -H "X-API-Key: $KEY" "http://localhost:8080/v1/memories?limit=1"`, then skip to plugin install

Cross-platform skill install: `npx skills add mingxy-cerebro/cerebro-server --skill cerebro -g`

### Platform quick reference

| Platform | Install | Config |
|----------|---------|--------|
| Claude Code | `/plugin marketplace add mingxy-cerebro/cerebro-server` | `~/.claude/settings.json` env field |
...
For detailed per-platform instructions (config formats, restart, verification, China network mirrors), READ the selfhost-setup reference.
...
> **Common failure:** Agents finish technical setup but forget the handoff message. Treat it as part of setup, not optional follow-up. For the full handoff template, READ `references/selfhost-setup.md`.
...
Cerebro organizes memories into three-tier Spaces for collective intelligence:
...
- Use plain product language, not backend vocabulary. Prefer "API key" or "Cerebro API key".
- Explain that the same API key reconnects the same memory server on another trusted machine.
...
Base: `http://localhost:8080` (default self-hosted). Auth: `X-API-Key` header.
