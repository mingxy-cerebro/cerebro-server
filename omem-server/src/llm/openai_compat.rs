...
impl OpenAICompatLlm {
    ...
    pub fn new_profile(config: &OmemConfig) -> Result<Self, OmemError> {
        let base_url = config.profile_llm_base_url.trim_end_matches('/');
        if base_url.is_empty() {
            return Err(OmemError::Llm(
                "profile_llm_base_url is required for openai-compatible provider".to_string(),
            ));
        }

        let mut headers = HeaderMap::new();
        headers.insert(CONTENT_TYPE, HeaderValue::from_static("application/json"));
        if !config.profile_llm_api_key.is_empty() {
            let auth_value = format!("Bearer {}", config.profile_llm_api_key);
            headers.insert(
                AUTHORIZATION,
                HeaderValue::from_str(&auth_value)
                    .map_err(|e| OmemError::Llm(format!("invalid api key header: {e}")))?,
